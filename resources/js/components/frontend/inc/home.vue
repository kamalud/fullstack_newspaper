    ...
    <div class="home">
            <div class="First_news">
                ...
                <div class="col-md-8">
                    <div class="row">
                        <div class="col-md-8">
                            <div v-if="posts[0]" class="main_image">
                                <img :src="'/upload/posts/'+ posts[0].image" alt="">
                                <h3>{{posts[0].title}}</h3>
                                <p><span v-html="StrLimit(posts[0].description,250)"></span></p>
                               <router-link :to="`/post-details/${posts[0].slug}`" class="btn btn-primary">Read more ..</router-link>
                           </div>
                        </div>
                        <div class="col-md-4">
                            <div class="sub_image" v-for="(post,i) in post4_list" :key="i">
                                <div class="row">
                                    <div class="col-md-4">
                                       <img :src="'/upload/posts/'+ post.image" alt="">
                                    </div>
                                    <div class="col-md-8">
                                       <h5><router-link :to="`/post-details/${post.slug}`">{{post.title}}</router-link></h5>
                                    </div>
                                </div>
                            </div>
                        </div>
